...
for i in range(0,times):
    ...
    ridgeR = Ridge(alpha=1000).fit(Xtrain_poly, ytrain)
    dummy = DummyRegressor(strategy='mean').fit(Xtrain_poly, ytrain)
    ypred = ridgeR.predict(Xtest_poly)
...
print('square error: Ridge regression with alpha=1:'+str(modelSq/times)+' dummy:'+str(dummySq/times))
